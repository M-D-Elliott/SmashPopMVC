...
namespace SmashPopMVC.Areas.Async.Controllers
{
    ...
    public class ApplicationUserController : Controller
    {
        ...
        public async Task<IActionResult> Profile(string id)
        {
            ...
            var user = await _applicationUserService.Get(id, social: true);

            if (user != null)
            {
                var model = await BuildUserProfile(user);
        ...
        public async Task<IActionResult> Search(string query, bool byMain, bool byAlt, bool byScore)
        {
            ...
            model = await CompleteSearchModel(model);
            if(model.Total == 1)
            {
        ...
        public async Task<IActionResult> Results(UserSearchModel model)
        {
        ...
        public async Task<IActionResult> Update(UpdateViewModel viewModel)
        {
            ...
            var user = await _applicationUserService.Get(viewModel.UserID);
            if (user == null)
            {
            ...
            {
                var new_main = await _characterService.GetByID(viewModel.MainID);
                user.Main = new_main;
                update = true;
            }
            if (user.Alt?.ID != viewModel.AltID)
            {
                var new_alt = await _characterService.GetByID(viewModel.AltID);
                user.Alt = new_alt;
                update = true;
            }
            ...
            if(user.ShortName != shortName && shortName != null)
            {
                ...
                {
                    var shortNameExists = await _applicationUserService.ShortNameExists(shortName);
                    if (!shortNameExists)
                    {
            ...
            if (update)
            {
                await _applicationUserService.Update(user.Id, user);
        ...
        private async Task<UserSearchModel> CompleteSearchModel(UserSearchModel model, int take = 20, int page = 1)
        {
            var users = await _applicationUserService.SearchUsers(model.SearchQuery, model.ByMain, model.ByAlt, model.ByScore);
        ...
        private async Task<ProfileIndexModel> BuildUserProfile(ApplicationUser user)
        {
            var currentUserID = await GetCurrentUserID();
            ...
            return new ProfileIndexModel
            {
                ...
                Friends = friends,
                Comments = await _commentPackager.BuildCommentListing(user.Id, currentUserID),
